...
DateFormat();
DateFormat& operator=(const DateFormat&);//to avoid memory leaks in copy assignment of dateformat in set format
friend ostream& operator<<(ostream&,const DateFormat&);//to verify that the output is correct .Not necessary in design! 
~DateFormat();
char* getdateFormat();
char* getmonthFormat();
char* getyearFormat();
...
void InvalidArgument(int d,int m,int y)
throw(invalid_argument);
void DomainError(int d,int m,int y)
throw(domain_error);
void OutOfRange(int d,int m,int y)
throw(out_of_range);
private:
