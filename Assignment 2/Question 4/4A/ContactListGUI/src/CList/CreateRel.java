...
public class CreateRel extends JFrame {
	 ...
	 * Create the frame.
	 */
	public CreateRel(FullContactList L) {
		setTitle("Create new relative");
		...
		btnDone .addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				int flag=1;
			      ...
			      Date d2 =new Date();
			      try{
			      d2=df.parse(textField_4.getText());
			      if(d2.compareTo(d1)<0)
			      {flag=0;JOptionPane.showMessageDialog(null,"Invalid Date","Input Error",JOptionPane.PLAIN_MESSAGE);dispose();}
			      }catch(Exception e){flag=0;JOptionPane.showMessageDialog(null,"Invalid Date","Input Error",JOptionPane.PLAIN_MESSAGE);dispose();}
			...
			{
				int i;
				relatives c1=new relatives(d1,d2,s1,em,mob);
				for(i=L.rel;i>0;--i)
				{
					if((c1.name).compareTo(L.r[i-1].name)<0)
						L.r[i]=L.r[i-1];
					else break;
