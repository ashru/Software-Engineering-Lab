...
public class CreateCas extends JFrame {
	...
	private JLabel lblOtherUsefulInfo;
	 ...
	 * Create the frame.
	 */
	public CreateCas(FullContactList L) {
		setTitle("Create new casual acquaintance");
		...
		JLabel lblBirthday = new JLabel("Date of Meeting");
		...
		JLabel lblDateOfLast = new JLabel("Place of Meeting");
		...
		lblNotableEvents = new JLabel("Circumstances of Meeting");
		...
		lblOtherUsefulInfo = new JLabel("Other Useful Info");
		GridBagConstraints gbc_lblOtherUsefulInfo = new GridBagConstraints();
		gbc_lblOtherUsefulInfo.anchor = GridBagConstraints.EAST;
		gbc_lblOtherUsefulInfo.insets = new Insets(0, 0, 5, 5);
		gbc_lblOtherUsefulInfo.gridx = 1;
		gbc_lblOtherUsefulInfo.gridy = 7;
		contentPane.add(lblOtherUsefulInfo, gbc_lblOtherUsefulInfo);
		...
		btnDone .addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				int flag=1;
			      ...
			      String s1=textField.getText();String s2="",s3="",s4="";
			      long mob=0;
			      try{
			...
			if(flag==1)
			{
				casualAq c1=new casualAq(s2,s3,s4,d1,s1,em,mob);
				int i,j;
				for(i=L.cas;i>0;--i)
				{
					if((c1.name).compareTo(L.c[i-1].name)<0)
						L.c[i]=L.c[i-1];
					else break;
